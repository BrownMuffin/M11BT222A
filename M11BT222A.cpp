...
void M11BT222A::showCharacter(uint8_t seg, char character)
{	
	if (character < 32 || character > 127) character = 127;

	uint16_t data = charData[(uint8_t)character - 32];
	
	showCharacterCustom(seg, data);
}

void M11BT222A::showCharacter(uint8_t seg, uint8_t character)
{	
	if (character < 32 || character > 127) character = 127;
